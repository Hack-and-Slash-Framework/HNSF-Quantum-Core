...
public class UModFighterDefinition : IFighterDefinition, IOnUModPrebuild
{
    ...
    public override async UniTask<bool> Load(string id)
    {
        ...
        if (commandList)
        {
            var commandListLoadHandle = modHost.Assets.LoadAsync(modAsset.ConvertIDToAssetPath(commandList.reference.assetID));
    ...
    public override async UniTask<bool> LoadAssets()
    {
        ...
        try
        {
            ...
            if(!fighterLoadHandle.IsSuccessful) throw new Exception($"Failed to load fighter. {fighterRef.ToString()}");
            fighterHandle = fighterLoadHandle;

            var fighterQuantumLoadHandle = modHost.Assets.LoadAsync(modAsset.ConvertIDToAssetPath(fighterQuantumRef.assetID));;
            if (!fighterQuantumLoadHandle.IsSuccessful) throw new Exception($"Failed to load quantum fighter. {fighterQuantumRef.ToString()}");
