...
namespace HnSF
{
    ...
    public class TimelineAssetOverride
    {
        ...
        public void SetOverridesOnly(PlayableDirector director, ITimelineDirectorBindingSource bindingSource)
        {
            for (int i = 0; i < Bindings.Length; i++)
            {
                if ((Overrides != null && !Overrides[i]) || !Bindings[i]) continue;
                ...
                {
                    director.SetGenericBinding(track, bindingSource.GetMapping(Bindings[i] as Tag));
                }
            }
        }
    }
