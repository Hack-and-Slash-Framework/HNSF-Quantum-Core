...
namespace HnSF
{
    public class GameplayTagsListElement : VisualElement
    {
        ...
        public UnityEvent<Tag> OnAddSubTagRequest = new UnityEvent<Tag>();
        ...
        public void Initialize(GameplayTagsSource tagsSource)
        {
            ...
            Action<VisualElement, int> bindItem = (e, i) =>
            {
                ...
                toggle.label = $"{item.tagAsset.label}";
        ...
        private void BuildTagMap(GameplayTagsSource tagsSource)
        {
            ...
            for (int i = 0; i < baseTags.Count; i++)
            {
                ...
                {
                    var childTag = QuantumUnityDB.GetGlobalAssetEditorInstance(baseTags[i].childTags[w]);
                    trd.childTags[w] = BuildChildMapRecursive(childTag, ref idCnt);
                    trd.childTags[w].parent = trd;
                }
            }
        }

        private GameplayTagListDataObject.TagReferenceData BuildChildMapRecursive(Tag tagAsset, ref int idCnt)
        {
            if (tagAsset == null) return null;
            var trd = new GameplayTagListDataObject.TagReferenceData()
            {
                id = idCnt++,
                selected = false,
            ...
            for (int w = 0; w < tagAsset.childTags.Count; w++)
            {
                ...
                var childTag = QuantumUnityDB.GetGlobalAssetEditorInstance(tagAsset.childTags[w]);
                trd.childTags[w] = BuildChildMapRecursive(childTag, ref idCnt);
                if (trd.childTags[w] == null) break;
                trd.childTags[w].parent = trd;
            }
