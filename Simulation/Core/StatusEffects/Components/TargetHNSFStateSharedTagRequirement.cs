...
{
    [System.Serializable]
    public unsafe partial class TargetHNSFStateSharedTagRequirement : StatusEffectComponent
    {
        [System.Serializable]
        ...
        private bool ValidateStateRequirements(Frame frame, StatusEffector* statusEffector, TagRequirements requirements)
        {
            ...
            if (!requirements.validTagsSet.Contains(currentStateAsset.sharedStateTag))
                return false;

            return !requirements.mustNotHaveTagsSet.Contains(currentStateAsset.sharedStateTag);
        }
    }
}