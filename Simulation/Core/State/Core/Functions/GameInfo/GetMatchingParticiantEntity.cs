...
{
    [System.Serializable]
    public unsafe partial class GetMatchingParticipantEntity : StateFunctionEntityRef
    {
        ...
        public bool checkIfExistsInTaggedEntityMap;
        
        public override EntityRef Execute(Frame frame, EntityRef entity, ref HNSFStateContext stateContext)
        {
            var selfParticipantLink = frame.Unsafe.GetPointer<ParticipantLink>(entity);
            ...
            foreach (var participantDataIntToEntity in participantDataEntities)
            {
                ...
                if(checkIfExistsInTaggedEntityMap && EntityInMap(frame, entity, participantActorEntityList[0])) continue;

                return participantActorEntityList[0];
            }
            
            return EntityRef.None;
        }

        private bool EntityInMap(Frame frame, EntityRef entity, EntityRef participantActorEntity)
        {
            if (!frame.Unsafe.TryGetPointer<TaggedEntityMapping>(entity, out var tem)) return false;
