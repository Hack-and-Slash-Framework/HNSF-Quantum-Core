...
{
    [Serializable]
    public unsafe partial class ActorCurrentStateType : ICondition
    {
        public string Label
        {
            get => label;
        ...
        public bool IsValid(ref HTNAgentContext context)
        {
                ...
                && frame.TryFindAsset(gsm->stateAgent.stateData.state, out var currentState))
            {
                ...
                {
                    if (currentState.stateType == validTag)
                    {
                        valid = true;
                        break;
                    }
                ...
                {
                    if (currentState.stateType == invalidTag)
                        return false;
                }
            }
            return false;
...
namespace HnSF.core.AI.HTN.Nodes
{
    ...
    public unsafe class ConditionActorCurrentStateType : ConditionBase
    {
        ...
        public override ICondition Convert()
        {
            ...
            return new Conditions.ActorCurrentStateType()
            {
                //Label = label,
